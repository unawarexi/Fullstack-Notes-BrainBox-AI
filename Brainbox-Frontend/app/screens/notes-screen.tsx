...
import { ChevronLeft, Undo2, Redo2, Check, Mic, Sparkles, Image as ImageIcon, Video, Music, FileText, Bold, Italic, List, Link, Layers, MoreHorizontal } from "lucide-react-native";
...
const NotesScreen = () => {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="flex-1 bg-white">
      {/* App Bar */}
      <View className="pt-12 px-4 pb-4 bg-white border-b border-gray-100">
        <View className="flex-row items-center justify-between">
          {/* Left: Back Button and Title */}
          <View className="flex-row items-center flex-1">
            <TouchableOpacity onPress={() => router.back()} className="w-10 h-10 items-center justify-center mr-3" activeOpacity={0.7}>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View className="flex-1 px-6">
          <TextInput className="text-base text-gray-800 leading-6" placeholder="Start writing your thoughts..." placeholderTextColor="#999" value={content} onChangeText={handleContentChange} multiline textAlignVertical="top" style={{ minHeight: 400 }} />
        </View>
      </ScrollView>
      ...
      <View className="px-6 py-2 bg-white border-t border-gray-100">
        <View className="flex-row items-center justify-between px-6 py-4">
          ...
          <View className="flex-row items-center gap-3">
            {/* Voice Input Button */}
            <TouchableOpacity className="w-12 h-12 bg-gray-900 rounded-full items-center justify-center shadow-sm" activeOpacity={0.8}>
              <Mic size={20} color="#fff" />
            </TouchableOpacity>

            {/* AI Assist Button */}
            <TouchableOpacity className="px-4 py-3 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl flex-row items-center gap-2 border border-gray-200" onPress={handleAIAssist} activeOpacity={0.7}>
          ...
          <TouchableOpacity className="w-10 h-10 items-center justify-center bg-gray-100 rounded-xl" activeOpacity={0.7} onPress={() => setSheetVisible(true)}>
